...
void InitializePopulation(long double *, int , double , int *, int);
int ChooseVictim(long double *, int , long double);
...
void main(int argc, char *argv[]) {
	...
	for (i = 0; i < generations; i++) {
		for (j = 0; j < popsize; j++) {
			currentvictim = ChooseVictim(wholepopulationdeathratestree, popsize, sumofdeathrates);
...
/*All Fenwick tree functions from Wikipedia sample implementation.*/
...
int ChooseVictim(long double *wholepopulationdeathratestree, int popsize, long double sumofdeathrates)
{
